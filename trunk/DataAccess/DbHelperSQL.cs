...
namespace DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static string connectionString
        {
            get
            {
                string databasePath = ShopUtil.XmlCOM.ReadConfig("DatabasePath");
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int ExecuteSqlNum(string SQLString)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                using (OleDbCommand cmd = new OleDbCommand(SQLString, connection))
                {
                    try
                    {
                        connection.Open();
                        int rows = 0;
                        int.TryParse(cmd.ExecuteScalar().ToString(), out rows);
                        return rows;
                    }
                    catch (System.Data.OleDb.OleDbException e)
                    {
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static object ExecuteScalar(string SQLString,string retrunSQL,params OleDbParameter[] cmdParms)
        {
            using (OleDbConnection connection = new OleDbConnection(connectionString))
            {
                using (OleDbCommand cmd = new OleDbCommand())
