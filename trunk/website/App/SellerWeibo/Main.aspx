...
<body>
   ...
   <%--  <a href="#" class="usertypea">区别呢?</a>--%>
     ...
     <div >
     <table class="usertype" style='<%=UserType()==0?"display:block":"" %>'>
     <tr>
     <th>用户类型</th><th>发微博数量</th><th>绑定帐号个数</th><th>保存自定义模版</th>
     </tr>
     <tr>
     <td>免费用户:</td><td>每天3个</td> <td>可绑定2个账户</td><td>不可以</td>
     </tr>
     <tr>
     <td>
...
<div class="wrapper mainb main">
    ...
    <div class="category">
     <asp:Repeater ID="rptAccessWeibo" runat="server">
            <ItemTemplate>
             ...
             <a href="<%#Eval("access_weibo_url")%>" target="_blank"><%#Eval("weibo_name")%></a>
            <br />
            <%#!CheckIsExpired(Eval("AccessTokenExpires"))?
                                "<a class='link-accessweibo' href='?a=unbind&id="+Eval("access_weibo_id")+"'>点击取消绑定"+Eval("platform_name")+"</a>":
                                                                               "<a class='link-rebind' style='color:red;' href='?a=rebind&id=" + Eval("access_weibo_id") + "&pltid=" + Eval("id") + "'>已过期，请点击重新绑定" + Eval("platform_name") + "</a>"
               %>
             </h3>
             </ItemTemplate>
            </asp:Repeater>
    ...
    <h2 class="hbline hbline1"><strong>推广产品列表</strong></h2>
        <ul id="list">
        <li class="first-item"><input  type="checkbox" class="chk-all" />全选 <input type='button' value='批量发微博' class='btn-all'/></li>
 ...
 <div>
 <span class="msg-red">TIP:不可选的表示帐号授权已经过期，请在点击页面左边红色链接重新绑定帐号。</span><br>
 <asp:Repeater ID="rptWeibo" runat="server">
 <ItemTemplate> 
 <input  type="radio" id="weibo-<%#Eval("id") %>" title="<%#Eval("platform_name") %>" class="bind-weibo"  <%#CheckIsExpired(Eval("AccessTokenExpires"))?"disabled='disabled'":"checked='checked'" %>  name="weibo" value="<%#Eval("accesskey") %>|<%#Eval("accesssecret") %>|<%#Eval("accesstoken") %>"/><%#Eval("platform_name") %>【<%#Eval("weibo_name") %>】
 </ItemTemplate>
 </asp:Repeater>
 </div>
</div>  
