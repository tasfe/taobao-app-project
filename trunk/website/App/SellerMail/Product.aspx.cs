...
public partial class pages_Default : Shop.ShopUI.BasePage
{
    ...
    public string SendGMail(string[] to, string subject, string content, string sendFrom, string sendPwd,out int sendCount)
    {
        ...
        {
            clint.Send(onemail);
            ShopUtil.LogInfo.FileLogPath = Server.MapPath("~/sellermail/log");
            ShopUtil.LogInfo.LogFileName = "sendmail.log";
            ShopUtil.LogInfo.WriteLog(string.Format("【{0}】【{1}】\r{2}", myEmail, myPwd, onemail.To.ToString()));
        }
        catch (Exception ex)
        {
            ShopUtil.LogInfo.FileLogPath = Server.MapPath("~/sellermail/log");
            ShopUtil.LogInfo.LogFileName = "failedmail.log";
            ShopUtil.LogInfo.WriteLog(string.Format("【{0}】【{1}】---\r{2}", myEmail, myPwd, ex.ToString()));
            return ex.ToString();
        }
        return "";
    }
