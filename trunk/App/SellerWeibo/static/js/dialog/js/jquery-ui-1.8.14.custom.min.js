...
$(function () {
        ...
        buttons: {
            "确认发送": function () {
                ...
                if (bindItem.attr("title").indexOf("腾讯") != -1) {
                    itemIndex = 0;
                    SendTx(checkedItem, key, secret);
                } else if (bindItem.attr("title").indexOf("新浪") != -1) {
                    itemIndex = 0;
                    SendSina(checkedItem, key, secret);
                }
            }
        }
    });
    ...
    var itemIndex = 0;
    function SendTx(checkedItem, key, secret) {
        if (itemIndex <= checkedItem.length - 1) {
            ...
            $.ajax({
                ...
                data: "a=2&c=" + encodeURIComponent(wTemplate) + "&p=" + encodeURIComponent(picPath) + "&k=" + key + "&s=" + secret,
                success: function (result) {
                    itemIndex++;
                    if (result.msg == "ok") {
                        $("#msg .send-msg").append("<div class='msg-green'>发送到腾讯微薄成功!</div>");
                        setTimeout(function () { SendTx(checkedItem, key, secret) }, 5000);
                    }
                    else if (result.msg == "server error") {
                        if (result.errcode == 10) {
                            $("#msg .send-msg").append("<div class='msg-red'>发送到腾讯微薄失败,发送频率太快,请稍后再试!</div>")
                        }
                        else {
                            $("#msg .send-msg").append("<div class='msg-red'>发送到腾讯微薄失败!</div>")
                        }
                    }
    ...
    function SendSina(checkedItem, key, secret) {
        if (itemIndex <= checkedItem.length - 1) {
            var wTemplate = $("#dialog .t:checked").val();
            if ($("#dialog .r-custom").attr("checked")) {
                wTemplate = $("#dialog .t-custom").val();
            ...
            $.ajax({
                ...
                data: "a=3&c=" + encodeURIComponent(wTemplate) + "&p=" + encodeURIComponent(picPath) + "&k=" + key + "&s=" + secret,
                success: function (result) {
                    itemIndex++;
                    var retID = 0;
                    try {
                        retID = $(result).find("id").text();
                    }
                    catch (e) {
                        retID = 0;
                    ...
                    if (retID != "0") {
                        $("#msg .send-msg").append("<div class='msg-green'>发送到新浪微薄成功!</div>");

                        setTimeout(function () { SendSina(checkedItem, key, secret) }, 5000);
                    }
                    else {
                        eval("var errorJson=" + result);
                        if (errorJson.msg == "no") {
                            $("#msg .send-msg").append("<div class='msg-red'>" + errorJson.detail + "</div>")
                        }
                    }
                }
            })
