$(function () {
    $('#dialog').dialog({
        ...
        buttons: {
            "确认导出": function () {
                ...
                var postData = "action=import&tids={0}&cols={1}&format={2}";
                postData = postData.replace("{0}", tradeIds).replace("{1}", import_cols).replace("{2}", impoartFormat)
    ...
    $(".btn-rateimport").click(function () {
        if ($(this).attr("isfreeuser") != "0") {
            ShowMsg("亲，免费用户不能导出哦，赶紧<a style='color:red' target='_blank' href='http://fuwu.taobao.com/item/subsc.htm?items=ts-13815-4:1'>点击</a>订购吧，只要￥5元");
        }
    ...
    $(".btn-mail").click(function () {
        if ($("#list .chk-item:checked").length == 0) {
            ShowMsg("请至少选择一个交易记录来获取发生过交易的老客户的邮箱信息。");
        }
        else {
            var tradeIds = "";
            $("#list .chk-item:checked").each(function () {
            ...
            window.location = "Product.aspx?tids=" + tradeIds;
        }
    })
})