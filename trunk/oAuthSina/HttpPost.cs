...
namespace LeoShi.Soft.OpenSinaAPI
{
    public class HttpPost : BaseHttpRequest
    {
        ...
        {
            //if (System.Configuration.ConfigurationManager.AppSettings["Proxy"] == "1")
            //{
        ...
        public string RequestUser(string userID,string ip)
        {
            string uploadApiUrl = "http://api.t.sina.com.cn/users/show.xml";
            //var uploadApiUrl = url;
            string postData = "id="+userID;
            //var status = postData.Split('=').GetValue(1).ToString();
            var appendUrl = AppendPostDataToUrl(postData, uploadApiUrl);
            var authorizationHeader = GetAuthorizationHeader(appendUrl, POST);
            ...
            var bytes = GetContentsBytes(request, userID);
        ...
        private byte[] GetContentsBytes(WebRequest request, string id)
        {
            string boundary = Guid.NewGuid().ToString();
            string header = string.Format("--{0}", boundary);
            string footer = string.Format("--{0}--", boundary);
            ...
            request.ContentType = string.Format("multipart/form-data; boundary={0}", boundary);
            contents.AppendLine(header);
            contents.AppendLine(String.Format("Content-Disposition: form-data; name=\"{0}\"", "id"));
            contents.AppendLine("Content-Type: text/plain; charset=US-ASCII");
            contents.AppendLine("Content-Transfer-Encoding: 8bit");
            contents.AppendLine();
            contents.AppendLine(id);
