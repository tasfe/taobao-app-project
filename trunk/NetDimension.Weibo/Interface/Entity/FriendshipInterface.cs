...
namespace NetDimension.Weibo.Interface.Entity
{
	...
	public class FriendshipInterface : WeiboInterface
	{
		...
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.Collection Friends(string uid , string screenName , int count , int cursor , bool trimStatus )
		{
		...
		/// <param name="cursor">返回结果的游标，下一页用返回值里的next_cursor，上一页用previous_cursor，默认为0。</param>
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.IDCollection FriendIDs(string uid , string screenName , int count , int cursor )
		{
		...
		/// <returns></returns>
		public IEnumerable<NetDimension.Weibo.Entities.user.Entity> FriendsInCommon(string uid, string suid , int count , int page )
		{
		...
		/// <returns></returns>
		public IEnumerable<NetDimension.Weibo.Entities.user.Entity> FriendsOnBilateral(string uid, int count , int page , bool sort )
		{
		...
		/// <param name="sort">排序类型，0：按关注时间最近排序，默认为0。 </param>
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.IDCollection FriendsOnBilateralIDs(string uid, int count, int page , bool sort )
		{
		...
		/// <param name="trimStatus">返回值中user字段中的status字段开关，0：返回完整status字段、1：status字段仅返回status_id，默认为1。 </param>
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.Collection Followers(string uid , string screenName , int count , int cursor, bool trimStatus )
		{
		...
		/// <param name="cursor">返回结果的游标，下一页用返回值里的next_cursor，上一页用previous_cursor，默认为0。 </param>
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.IDCollection FollowerIDs(string uid , string screenName , int count , int cursor )
		{
		...
		/// <returns></returns>
		public IEnumerable<NetDimension.Weibo.Entities.user.Entity> FollowersInActive(string uid, int count )
		{
		...
		/// <param name="page">返回结果的页码，默认为1。</param>
		/// <returns></returns>
		public NetDimension.Weibo.Entities.user.Collection FriendsChain(string uid, int count , int page )
		{
