...
namespace NetDimension.Weibo
{
	...
	public class OAuth
	{
		...
		public OAuth(string appKey, string appSecret, string accessToken, string refreshToken )
		{
			this.AppKey = appKey;
			this.AppSecret = appSecret;
		...
		internal string Request(string url, RequestMethod method, bool multi , params WeiboParameter[] parameters)
		{
		...
		/// </param>
		/// <returns></returns>
		public string GetAuthorizeURL(ResponseType response,string state, DisplayType display )
		{
        ...
        {
            ResponseType response=ResponseType.Code;
            string state="";
		...
		public TokenResult VerifierAccessToken()
		{
			try
			{
				var json = Request("https://api.weibo.com/2/account/get_uid.json", RequestMethod.Get,false);
